...
However! This is where Must-Use Return Values come in. The feature can finally provide compiler guidance here and
...
Why is the return value considered unused? Because apply coerces the result of its lambda to `Unit`. Its signature is:
`T'.apply(block: T'.() -> Unit): T'`

So what should you do instead? Well, depends on your case! If you intentionally call `computeResult()` only for its side effects, you can make that explicit:
[source,kotlin]
----
expr.apply {
...
That was a quick overview of the Must-Use Return Values feature. After enabling it, we found quite a few bugs in both the Kotlin compiler and our libraries, so I encourage you to give it a try.
